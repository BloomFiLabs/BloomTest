...
// --- Strategy Config: WBTC/USDT Hybrid Mode ---
const STRATEGY_CONFIG = {
    mode: StrategyMode.HYBRID,
    poolFee: 0.003, // 0.3% Fee
    targetAPR: 0.30,
    allocation: 1000000,
    checkIntervalHours: 17 // Hybrid mode: 17h checks
};
...
async function loadHourlyHistory(pair: string, start: Date, end: Date): Promise<any[]> {
    ...
    // WBTC/USDT 0.3% pool address
    const adapter = new TheGraphDataAdapter({
        subgraphUrl: url,
        token0Symbol: 'WBTC',
        token1Symbol: 'USDT',
        poolAddress: '0x9db9e0e53058c89e5b94e29621a205198648425b' // WBTC/USDT 0.3%
    });
...
async function main() {
    console.log(`\n🧪 Qualifying WBTC/USDT Strategy: HYBRID Mode`);
    ...
    let hourlyData = [];
    try {
        hourlyData = await loadHourlyHistory('WBTC-USDT', start, end);
    } catch (e) {
    ...
    const strategy = new VolatilePairStrategy('wbtc-usdt-hybrid', 'WBTC/USDT Hybrid Strategy');
    ...
    console.log(`   Expected Behavior: Hybrid mode should rebalance moderately (1-2/day typically)`);
    
    if (rebalanceCount/90 >= 0.5 && rebalanceCount/90 <= 3.0) {
        console.log(`   ✅ PASSED: Hybrid mode frequency is within expected range.`);
    } else {
        console.log(`   ⚠️ WARNING: Rebalance frequency seems outside expected range for Hybrid mode.`);
    }
}
