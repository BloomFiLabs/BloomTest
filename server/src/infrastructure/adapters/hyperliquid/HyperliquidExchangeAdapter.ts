...
export class HyperliquidExchangeAdapter implements IPerpExchangeAdapter {
  ...
  private readonly BALANCE_CACHE_TTL = 2000; // 2 seconds cache during trading (reduced to prevent stale data race conditions)
  ...
  constructor(
    ...
    if (!privateKey) {
      throw new Error(
        'Hyperliquid exchange requires PRIVATE_KEY or HYPERLIQUID_PRIVATE_KEY',
      );
    }
  ...
  /**
   * Enable active trading mode for shorter cache TTL
   * Call this before placing orders
   */
  setActiveTradingMode(active: boolean): void {
    this.isActiveTradingMode = active;
    if (active) {
      // Clear balance cache when entering trading mode
  ...
  async placeOrder(request: PerpOrderRequest): Promise<PerpOrderResponse> {
    // Enable active trading mode for shorter cache TTL
    this.setActiveTradingMode(true);
    ...
    try {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
          ...
          if (parseFloat(formattedPrice) <= 0) {
            throw new Error(
              `Invalid order price: ${formattedPrice} (original: ${orderPrice})`,
            );
          }
          ...
          if (
            ...
            if ('error' in status && status.error) {
              ...
              const isRetryable =
                errorMsg.includes('could not immediately match') ||
            ...
            throw new Error(
              `Unexpected order response format: ${JSON.stringify(result).substring(0, 200)}`,
            );
          }
        } catch (error: any) {
          lastError = error;
      ...
      if (!result || result.status !== 'ok') {
        throw (
          lastError ||
          new Error(`Order placement failed after ${MAX_RETRIES} attempts`)
        );
      }
        ...
        for (const assetPos of clearinghouseState.assetPositions) {
          ...
          if (Math.abs(size) > 0.0001) {
            ...
            const positionValue = parseFloat(
              assetPos.position.positionValue || '0',
            );
            const markPrice =
  ...
  private async getCoinFromAssetIndex(assetIndex: number): Promise<string> {
    return this.callInfo(this.WEIGHT_INFO_HEAVY, async () => {
      ...
      const asset = meta.universe?.find(
        (a: any, index: number) => index === assetIndex,
      );
      if (asset?.name) {
        return asset.name;
      }
  ...
  async cancelOrder(orderId: string, symbol?: string): Promise<boolean> {
    try {
      ...
      if (!symbol) {
        throw new Error(
          'Symbol is required for canceling orders on Hyperliquid',
        );
      }
      ...
      this.logger.debug(
        `Cancelling order ${orderId} for ${symbol} (asset index: ${assetId})`,
      );
      ...
      if (result.status === 'ok') {
        this.logger.log(
          `✅ Order ${orderId} cancelled successfully on Hyperliquid`,
        );
        return true;
      } else {
        this.logger.warn(
          `Failed to cancel order ${orderId}: ${JSON.stringify(result)}`,
        );
        return false;
      }
    } catch (error: any) {
  ...
  async cancelAllOrders(symbol: string): Promise<number> {
    try {
      ...
      this.logger.debug(
        `Cancelling all orders for ${symbol} (asset index: ${assetId})`,
      );
      ...
      if (result.status === 'ok') {
        this.logger.log(
          `✅ Cancelled ${cancels.length} order(s) for ${symbol} on Hyperliquid`,
        );
        return cancels.length;
      } else {
        this.logger.warn(
          `Failed to cancel all orders for ${symbol}: ${JSON.stringify(result)}`,
        );
        return 0;
      }
    } catch (error: any) {
    ...
    return this.callInfo(this.WEIGHT_INFO_LIGHT, async () => {
      ...
      if (!order) {
        ...
        if (fill) {
          ...
          const side =
            (fill as any).side === 'B' ? OrderSide.LONG : OrderSide.SHORT;
    ...
    try {
      ...
      if (l2Book && l2Book.levels) {
        ...
        if (bestBid > 0 && bestAsk > 0) {
          ...
          this.logger.debug(
            `Order book for ${symbol}: bid=${bestBid.toFixed(4)}, ask=${bestAsk.toFixed(4)}`,
          );
  ...
  async getMarkPrice(symbol: string): Promise<number> {
    ...
    try {
      ...
      this.logger.debug(
        `WebSocket mark price not available for ${symbol}, using REST API`,
      );
    }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const markPrice = parseFloat(
          (allMidsData as any)[baseCoin] || (allMidsData as any)[coin] || '0',
        );
        if (markPrice > 0) {
          // Cache the result
        ...
        throw new Error(
          `Mark price not found for ${symbol} (coin: ${coin}, baseCoin: ${baseCoin})`,
        );
      } catch (error: any) {
        lastError = error;
        if (attempt === 2) {
  ...
  async getBalance(): Promise<number> {
    ...
    // Use shorter TTL during active trading to prevent stale balance race conditions
    const cacheTTL = this.isActiveTradingMode
      ? this.BALANCE_CACHE_TTL
      : this.BALANCE_CACHE_TTL_IDLE;
    if (
      this.balanceCache &&
      Date.now() - this.balanceCache.timestamp < cacheTTL
    ) {
      const cacheAge = Math.round(
        (Date.now() - this.balanceCache.timestamp) / 1000,
      );
      this.logger.debug(
        `Using cached balance: $${this.balanceCache.balance.toFixed(2)} (age: ${cacheAge}s, mode: ${this.isActiveTradingMode ? 'trading' : 'idle'})`,
      );
  ...
  async getEquity(): Promise<number> {
    ...
    try {
      ...
      const cacheTTLFallback = this.isActiveTradingMode
        ? this.BALANCE_CACHE_TTL
        : this.BALANCE_CACHE_TTL_IDLE;
      if (
        this.balanceCache &&
        Date.now() - this.balanceCache.timestamp < cacheTTLFallback
      ) {
  ...
  async depositExternal(
    ...
    try {
      ...
      if (amount < 5) {
        throw new Error(
          `Deposit amount must be at least 5 USDC. Received: ${amount}`,
        );
      }
      ...
      if (asset.toUpperCase() !== 'USDC') {
        throw new Error(
          `Only USDC deposits are supported via Bridge2. Received: ${asset}`,
        );
      }

      this.logger.log(
        `Depositing $${amount.toFixed(2)} ${asset} to Hyperliquid via Bridge2...`,
      );

      // Bridge2 contract addresses
      const BRIDGE_CONTRACT_ADDRESS =
        '0x2df1c51e09aecf9cacb7bc98cb1742757f163df7'; // Arbitrum mainnet
      ...
      if (!privateKey) {
        throw new Error(
          'PRIVATE_KEY or HYPERLIQUID_PRIVATE_KEY required for deposits',
        );
      }
      ...
      if (currentAllowance < depositAmountWei) {
        this.logger.log(
          `Approving Bridge2 contract to spend ${depositAmount.toFixed(2)} USDC...`,
        );
  ...
  async withdrawExternal(
    ...
    try {
      const clearinghouseState = await this.callInfo(this.WEIGHT_INFO_LIGHT, () => this.infoClient.clearinghouseState({
        ...
        if (perpAvailable < totalRequired) {
          ...
          try {
            ...
            const updatedMarginUsed = parseFloat(
              updatedMarginSummary.totalMarginUsed || '0',
            );
            const updatedPerpAvailable =
              updatedAccountValue - updatedMarginUsed;
        ...
        this.logger.warn(
          `Failed to check/transfer balance before withdrawal: ${balanceError.message}`,
        );
      ...
      if (error.response?.status === 422) {
        errorMessage =
          `Invalid withdrawal request (422): ${error.response?.data?.message || error.message}. ` +
      ...
      this.logger.error(
        `Failed to withdraw ${assetDisplay} to ${destination}: ${errorMessage}`,
      );
      if (error.response?.data) {
        this.logger.debug(
          `Hyperliquid API error response: ${JSON.stringify(error.response.data)}`,
        );
      }
  ...
  async getFundingPayments(
    ...
    const response = await this.callInfo(this.WEIGHT_INFO_HEAVY, () => axios.post(
      ...
      if (!Array.isArray(data)) {
        this.logger.warn(
          `Unexpected userFunding response format: ${JSON.stringify(data).substring(0, 200)}`,
        );
        return [];
      }
      ...
      this.logger.debug(
        `Retrieved ${payments.length} funding payments from Hyperliquid`,
      );
      return payments;
    } catch (error: any) {
      ...
      return openOrders.map((order: any) => {
        ...
        return {
          ...
          filledSize:
            ...
            parseFloat(order.origSz || order.sz || '0'),
