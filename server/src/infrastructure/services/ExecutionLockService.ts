...
export interface ActiveOrder {
  ...
  threadId: string;
  placedAt: Date;
  status: 'PLACING' | 'PLACED' | 'WAITING_FILL' | 'FILLED' | 'FAILED' | 'CANCELLED';
  size?: number;
  price?: number;
}
...
export class ExecutionLockService {
  ...
  private readonly executingSymbols: Map<string, {
    startedAt: Date;
    threadId: string;
    operation: string;
  }> = new Map();
  ...
  tryAcquireGlobalLock(threadId: string, operation: string): boolean {
    ...
    this.logger.debug(`🔒 Global lock acquired by ${threadId} for ${operation}`);
    return true;
  }
   ...
   */
  tryAcquireSymbolLock(symbol: string, threadId: string, operation: string): boolean {
    ...
    this.logger.debug(`🔒 Lock acquired for ${normalizedSymbol} by ${threadId} (${operation})`);
    return true;
  }
  ...
  releaseSymbolLock(symbol: string, threadId: string): void {
    ...
    this.executingSymbols.delete(normalizedSymbol);
    this.logger.debug(`🔓 Lock released for ${normalizedSymbol} by ${threadId}`);
  }
   ...
   */
  private getOrderKey(exchange: string, symbol: string, side: 'LONG' | 'SHORT'): string {
   ...
   */
  hasActiveOrder(exchange: string, symbol: string, side: 'LONG' | 'SHORT'): boolean {
    const key = this.getOrderKey(exchange, symbol, side);
    const order = this.activeOrders.get(key);
   ...
   */
  getActiveOrder(exchange: string, symbol: string, side: 'LONG' | 'SHORT'): ActiveOrder | null {
    const key = this.getOrderKey(exchange, symbol, side);
    const order = this.activeOrders.get(key);
   ...
   */
  forceClearOrder(exchange: string, symbol: string, side: 'LONG' | 'SHORT'): void {
    const key = this.getOrderKey(exchange, symbol, side);
    const order = this.activeOrders.get(key);
