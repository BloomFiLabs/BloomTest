...
export class ExchangeBalanceRebalancer {
  ...
  async getExchangeBalances(
    ...
    for (const [exchangeType, adapter] of adapters) {
      try {
        ...
        this.logger.warn(`Failed to get balance from ${exchangeType}: ${error.message}`);
  ...
  calculateRebalancePlan(
    ...
    if (activeExchanges.size > 0) {
      ...
      targetBalance = activeExchanges.size > 0 
      ...
      this.logger.log(`No active opportunities - using average balance: $${targetBalance.toFixed(2)}`);
    }
    ...
    // Calculate totals
    const totalExcess = exchangesWithExcess.reduce((sum, info) => sum + info.excess, 0);
    ...
    const hasInactiveExcess = exchangesWithExcess.some(
      (info) => inactiveExchanges.has(info.exchange)
    );
    const hasActiveDeficit = exchangesWithDeficit.some(
      (info) => activeExchanges.has(info.exchange)
    );
    ...
    const differencePercent = targetBalance > 0 ? (balanceDifference / targetBalance) * 100 : 0;
    ...
    const canRebalance =
      ...
      (hasInactiveExcess || differencePercent >= this.REBALANCE_THRESHOLD_PERCENT);
  ...
  async executeRebalance(plan: RebalancePlan): Promise<RebalanceResult> {
    ...
    if (!plan.canRebalance) {
      this.logger.log('⏸️  Rebalancing not needed - balances are within threshold');
      return result;
    }
    ...
    while (excessIndex < excessQueue.length && deficitIndex < deficitQueue.length) {
  ...
  private async transferBetweenExchanges(
    ...
    try {
      ...
      this.logger.log(`✅ Deposit successful: ${depositTxHash || withdrawTxHash}`);
  ...
  private logPlan(plan: RebalancePlan): void {
    this.logger.log('\n📊 Rebalance Plan:');
    this.logger.log(`   Target balance (active exchanges): $${plan.targetBalance.toFixed(2)}`);
    
    if (plan.activeExchanges.size > 0) {
      this.logger.log(`   Active exchanges (with opportunities): ${Array.from(plan.activeExchanges).join(', ')}`);
    }
    
    if (plan.inactiveExchanges.size > 0) {
      this.logger.log(`   Inactive exchanges (no opportunities): ${Array.from(plan.inactiveExchanges).join(', ')}`);
      this.logger.log(`   → Will withdraw excess from inactive exchanges to fund active ones`);
    }
    
    if (plan.exchangesWithExcess.length > 0) {
      this.logger.log(`   Exchanges with excess:`);
      for (const info of plan.exchangesWithExcess) {
        const status = plan.inactiveExchanges.has(info.exchange) ? '(inactive - will withdraw)' : '(active)';
        this.logger.log(
